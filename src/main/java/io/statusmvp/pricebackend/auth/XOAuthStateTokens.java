...
/**
 * Stateless X OAuth state tokens.
 *
 * <p>When enabled (via {@code app.auth.x.stateSecret}), {@code /api/v1/auth/x/start} issues a signed
 * state token that contains the app redirect URI and a random nonce. The PKCE {@code code_verifier}
 * is deterministically derived from (secret, nonce) so {@code /api/v1/auth/x/callback} does not need
 * a shared Redis entry to validate the state.
 */
public final class XOAuthStateTokens {
  private static final Base64.Encoder B64URL = Base64.getUrlEncoder().withoutPadding();
  private static final Base64.Decoder B64URL_DEC = Base64.getUrlDecoder();
  private static final int SIG_BYTES_LEN = 32; // HMAC-SHA256
  private static final int VERSION = 1;
  private static final long MAX_FUTURE_IAT_SKEW_SECONDS = 60;
  private static final String SIG_INFO = "x-state-v1";
  private static final String PKCE_INFO = "x-pkce-v1";

  private XOAuthStateTokens() {}
  ...
  private static byte[] signInput(String payloadB64) {
    // Bind signature to a fixed domain separator to avoid cross-protocol key reuse mistakes.
  ...
  private static String derivePkceCodeVerifier(byte[] secret, String nonce) {
    byte[] digest =
        hmacSha256(secret, (PKCE_INFO + "|" + nonce).getBytes(StandardCharsets.UTF_8));
    // 32 bytes => 43 chars base64url (PKCE compliant).
